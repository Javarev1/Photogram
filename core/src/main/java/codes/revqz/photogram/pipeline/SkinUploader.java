...
import codes.revqz.photogram.util.Retry;
import codes.revqz.photogram.util.TextureCodec;
...
public final class SkinUploader {
    ...
    @Inject
    public SkinUploader(TextureStore textureStore, @Named("photogram") Logger logger) {
    ...
    private CompletableFuture<Void> processOneTile(ManagedImage image, Tile tile) {
        ...
        return Retry.withBackoff(() -> submitToMineSkin(tile), RETRY_LIMIT, RETRY_BASE_DELAY)
                .thenAccept(textureValue -> {
                    ...
                    int completed = image.completedUploads();
                    int total = image.totalTiles();
                    int percentage = (int) (((double) completed / total) * 100);
                    logger.info("[" + image.id() + "] Uploading tiles to MineSkin... " + completed + "/" + total + " ("
                            + percentage + "%)");
                });
    }
